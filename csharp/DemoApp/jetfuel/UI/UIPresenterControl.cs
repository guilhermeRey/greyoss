...
namespace EixoX.UI
{
    ...
    public class UIPresenterControl
    {
        ...
        public UIControlState GetState(object entity, bool validateRestrictions)
        {
            ...
            Dictionary<object, string> invalidated = null;

            if (entity is InvalidReady)
            {
                InvalidReady invalidReadyObj = (InvalidReady)entity;
                invalidated = invalidReadyObj.Invalidated;
            }

            string errorMessage = 
                 validateRestrictions && 
                _Restrictions != null && _Restrictions.Count > 0 ?
                (invalidated != null && invalidated.Count > 0 && invalidated.ContainsKey(_Member.Name) ?
                invalidated[_Member.Name] :
